...
class Animal {
  ...
  static async insertAnimal(newAnimal) {
    const query = {
  ...
  static async assignAnimal(animalId, compoundId) {
  ...
  static async assignVet(animalId, personalId) {
    const animalQueryResult = await pool.query('SELECT * FROM "Tier" WHERE id = $1', [animalId]);
    if (animalQueryResult.rowCount === 0) {
      throw new HTTPException(404, {
        message: `Animal with id ${animalId} does not exist!`
    ...
    const veterinarianQueryResult = await pool.query(`SELECT * FROM "Personal" WHERE id = $1 AND rolle = $2`, [personalId, "Tierarzt"]);
    if (veterinarianQueryResult.rowCount === 0) {
      throw new HTTPException(404, {
        message: `Veterinarian with id ${personalId} does not exist!`
      });
    }
    const vetAnimalCountResult = await pool.query(`SELECT COUNT(*) FROM "Tier" WHERE tierarzt_id = $1`, [personalId]);
    ...
    const updateResult = await pool.query('UPDATE "Tier" SET tierarzt_id = $1 WHERE id = $2 RETURNING *', [personalId, animalId]);
    if (updateResult.rowCount === 0) {
      throw new HTTPException(400, {
...
animal.get("/:id", async (c) => {
  try {
    ...
    const animal2 = await Animal.findById(id);
    if (!animal2) {
      return c.json({ message: `Kein Tier mit ID ${id} gefunden` }, 404);
    }
    return c.json({ data: animal2 }, 200);
  } catch (err) {
...
animal.post("/:id/assign-compound", async (c) => {
  try {
    ...
    if (isNaN(compoundId)) {
      return c.json({ message: "Query parameter 'compoundId' must be a valid number" }, 400);
    }
...
animal.post("/:id/assign-veterinarian", async (c) => {
  try {
    ...
    if (isNaN(personalId)) {
      return c.json({ message: "Query parameter 'personalId' must be a valid number" }, 400);
    }
...
// src/routes/employes.ts
var employe = new Hono2;
employe.get("/", async (c) => {
  try {
...
employe.get("/:id", async (c) => {
  try {
    const { id } = c.req.param();
    const employe2 = await Employe.findById(id);
    if (!employe2) {
      return c.json({ message: `Kein Mitarbeiter mit ID ${id} gefunden` }, 404);
    }
    return c.json({ data: employe2 }, 200);
  } catch (err) {
